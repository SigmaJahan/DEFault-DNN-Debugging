...
data = pd.read_csv('..\\regularizer_correct_data.csv')
df_combined = data.copy()
df_combined['label'] = df_combined['label'].apply(lambda x: 1 if x == 'Regularizer' else 0)
...
def evaluate_regulizer_sample(sample):
    """
