...
class MaskedConv2D(tfkl.Layer):
    ...
    def create_mask(self, in_ch, out_ch):
        k_y, k_x = self.kernel_size, self.kernel_size
        mid_x, mid_y = k_x // 2, k_y // 2

        mask_A = tf.concat([
            tf.ones([mid_y, k_x]),
            tf.concat([tf.ones([1, mid_x]), tf.zeros([1, k_x - mid_x])], axis=-1),
            tf.zeros([k_y - mid_y - 1, k_x])
        ], axis=0)

        mask_B = tf.concat([
            tf.ones([mid_y, k_x]),
            tf.concat([tf.ones([1, mid_x + 1]), tf.zeros([1, k_x - mid_x - 1])], axis=-1),
            tf.zeros([k_y - mid_y - 1, k_x])
        ], axis=0)

        mask = mask_B if self.type == 'B' else mask_A
        mask = tf.reshape(mask, (k_y, k_x, 1, 1))
        mask = tf.tile(mask, [1, 1, in_ch, out_ch])
        return tf.cast(mask, tf.float32)

    def call(self, x):
        mask = self.create_mask(x.shape[-1] // self.n_colors, self.filters // self.n_colors)
        h = tf.nn.conv2d(
            input=x,
            filters=self.kernel * mask,
