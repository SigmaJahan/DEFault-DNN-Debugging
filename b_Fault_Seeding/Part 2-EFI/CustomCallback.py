...
class EnhancedLoggingCallback(Callback):
    def __init__(self, train_dataset, filename, large_weight_threshold=10.0, acc_gap_threshold=0.1, oscillation_threshold=0.01, low_accuracy_threshold=0.5, max_threshold=5, min_threshold=-5, threshold_layer=0.5, relu_threshold=0.0, layer_threshold=0.7):
        ...
        with open(self.filename, 'w', newline='') as file:
            ...
            headers = ['epoch', 'train_loss', 'train_acc', 'val_loss', 'val_acc', 'large_weight_count',
                       ...
                       'nan_weight_count', 'nan_gradients_count', 'saturated_activation', 'mean_hvp', 'hvp_std',
                       'mean_gradient', 'gradient_std',
                       'hvp_max', 'hvp_min', 'hvp_median', 'gradient_max', 'gradient_min',
                       'gradient_median']
    ...
    def on_train_batch_end(self, batch, logs=None):
        ...
        for x_batch, y_batch in gradient_dataset:
            ...
            with tf.GradientTape() as hvp_tape:
                hvp_tape.watch(self.model.trainable_weights)
                with tf.GradientTape() as inner_tape:
                    inner_tape.watch(self.model.trainable_weights)
                    predictions = self.model(x_batch, training=True)
                    loss = self.model.compiled_loss(y_batch, predictions)
                grads = inner_tape.gradient(loss, self.model.trainable_weights)
            v = [tf.random.normal(shape=g.shape) for g in grads]
            dense_grads = [tf.convert_to_tensor(g) if isinstance(g, tf.IndexedSlices) else g for g in grads]
            hessian_vector = hvp_tape.gradient(dense_grads, self.model.trainable_weights, output_gradients=v)
            dataset_hvp.append(hessian_vector)
        ...
        average_hvp = [tf.reduce_mean(tf.stack([h[i] for h in dataset_hvp]), axis=0) for i in range(len(self.model.trainable_weights))]
        self.epoch_gradients.append(average_gradients)
        self.epoch_hvp.append(average_hvp)
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        average_hvp = [tf.reduce_mean(tf.stack([h[i] for h in self.epoch_hvp]), axis=0) for i in range(len(self.model.trainable_weights))]
        hvp_norms = [tf.norm(h).numpy() for h in average_hvp]
        mean_hvp = np.mean(hvp_norms)
        hvp_std = np.std(hvp_norms)
        hvp_max = np.max(hvp_norms)
        hvp_min = np.min(hvp_norms)
        hvp_median = np.median(hvp_norms)
        ...
        with open(self.filename, 'a', newline='') as file:
            ...
            writer.writerow([epoch, train_loss, train_acc, val_loss, val_acc, large_weight_count, acc_gap_too_big, loss_oscillation,
                 dying_relu,
                 gradient_vanish, gradient_explode, decrease_acc_count, increase_loss_count, cons_mean_weight_count,
                 cons_std_weight_count,
                 nan_weight_count, nan_gradients_count, saturated_activation, hvp_max, hvp_min, hvp_median,
                 mean_hvp, hvp_std, gradient_max, gradient_min, gradient_median, mean_gradient,
                 gradient_std])
    ...
    def on_train_end(self, logs=None):
        ...
        # Perform garbage collection at the end of training
        gc.collect()
