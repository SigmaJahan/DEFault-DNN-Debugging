...
#new add
def evaluate_and_explain_fault(sample):
    """
    Evaluates and explains a single sample, providing feature importance insights based on SHAP and LIME.
    
    Args:
        sample (pd.Series or array-like): Input features for a single sample, must match training features format.
    
    Returns:
        dict: Dictionary containing SHAP important features and corresponding fault insights.
    """
    ...
    if isinstance(shap_values, list):
        shap_values = shap_values[1] 
    shap_values = np.array(shap_values).reshape(1, -1)[:, :len(X.columns)]  
    shap_important_features = pd.DataFrame(shap_values, columns=X.columns).abs().mean().sort_values(ascending=False).head(5).index.tolist()
    ...
    explainer_lime = lime.lime_tabular.LimeTabularExplainer(
        X_train.values, feature_names=X.columns, class_names=['Correct', 'Buggy'], discretize_continuous=True
    )
    lime_exp = explainer_lime.explain_instance(sample_df.values[0], model.predict_proba, num_features=5)
    lime_features = [feature[0] for feature in lime_exp.as_list()]
    lime_faults = {feature: fault_messages.get(feature, "No specific fault message") for feature in lime_features}

    return {
        'shap_important_features': shap_important_features,
        'shap_fault_insights': shap_faults,
        'lime_important_features': lime_features,
        'lime_fault_insights': lime_faults
    }

sample = X_test.iloc[random_instance]
fault_analysis = evaluate_and_explain_fault(sample)
print("Fault Analysis:", fault_analysis)